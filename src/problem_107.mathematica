(* Project Euler Problem 107 - https://projecteuler.net/problem=107
   ...
   Given a weighted undirected network of 40 vertices encoded as an adjacency matrix with missing edges, compute the
   maximum total weight that can be removed while keeping the graph connected. This is the savings between the total
   weight of the original network and the weight of a minimum spanning tree.
 
   The reduction is classical: any connected weighted graph has a minimum spanning tree (MST) whose total weight is
   minimal among all spanning subgraphs, and removing all edges not in an MST yields the largest possible savings.
   Thus the task is to compute total edge weight and MST weight, then take their difference. With n=40, the edge set
   size is O(n^2); Kruskal's algorithm runs in O(E log E) time with a disjoint-set union structure, easily feasible.
 
   Parallelization is applied to the O(n^2) data reduction: each row's contribution to the upper-triangular edge list
   and to the total weight is independent, so ParallelTable partitions rows across kernels and the results are joined
   and summed via associative operations. The MST pass remains sequential but is negligible for this input size.
 
  Implementation sketch: embed the matrix as raw CSV lines, parse integers with 0 for missing edges, build
  upper-triangular weights and edge list in parallel, sort by weight, and run Kruskal with union-find. The final
  answer is total weight minus MST weight, returned by solve[].
 *)
...
solve[] := Module[{lines, matrix, weights, n, rowSums, totalWeight, edgesByRow, edges,
    sortedEdges, parent, rank, find, union, mstWeight, edgesUsed},
  lines = dataLines;
  matrix = Map[StringSplit[#, ","] &, lines];
  weights = Map[If[# == "-", 0, ToExpression[#]] &, matrix, {2}];
  n = Length[weights];
  rowSums = ParallelTable[Total[weights[[i, i + 1 ;; n]]], {i, 1, n - 1}];
  totalWeight = Total[rowSums];
  edgesByRow = ParallelTable[
    Select[Table[{i, j, weights[[i, j]]}, {j, i + 1, n}], #[[3]] > 0 &],
    {i, 1, n - 1}
  ];
  edges = Flatten[edgesByRow, 1];
  sortedEdges = SortBy[edges, Last];
  parent = Range[n];
  rank = ConstantArray[0, n];
  find[x_] := parent[[x]] = If[parent[[x]] == x, x, find[parent[[x]]]];
  union[x_, y_] := Module[{rx = find[x], ry = find[y]},
    If[rx == ry, False,
      If[rank[[rx]] < rank[[ry]],
        parent[[rx]] = ry,
        parent[[ry]] = rx;

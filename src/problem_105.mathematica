(* Project Euler Problem 105 — https://projecteuler.net/problem=105
   ...
   Given 100 candidate sets of distinct positive integers, find those that are special sum sets: for every pair of
   disjoint non-empty subsets B and C, sums differ and the larger subset has larger sum; sum all elements of every
   qualifying set and report the total across the file.
 
   For a sorted set a1<...<an, condition (ii) reduces to the prefix bound sum_{i=1}^{k+1} a_i > sum_{i=n-k+1}^n a_i
   for k <= floor((n-1)/2). This O(n) check prunes most failures. Condition (i) becomes uniqueness of all 2^n-1
   non-empty subset sums; with n <= 12, enumerating 4095 sums per set is cheap. Total work is ~100*4095 operations
   of integer addition and hashing, well within milliseconds; memory is linear in subset count per set.
 
   Parallelisation is straightforward: each set is independent. A ParallelMap over the dataset applies the two-phase
   predicate and returns either the set sum or 0; a final Total accumulates results. The reduction is associative,
   so no shared mutable state or synchronization hazards occur. Distribution overhead is negligible compared to the
   per-set subset enumeration.
 
   Implementation outline: embed the dataset as a literal list; define a predicate specialQ performing the prefix
   test then uniqueness check via subset sums; distribute definitions to subkernels; compute ParallelMap[
   If[specialQ[#], Total[#], 0]&, data] and sum. All arithmetic is exact integers; no I/O or randomness ensures
   determinism.
 *)
...
specialQ[s_List] := Module[{n = Length[s], a = Sort[s], sums},
  If[!And @@ Table[Total[a[[1 ;; k + 1]]] > Total[a[[n - k + 1 ;; n]]], {k, 1, Floor[(n - 1)/2]}], Return[False]];
  sums = Total /@ Rest[Subsets[a]];
  Length[Union[sums]] === Length[sums]
];

DistributeDefinitions[specialQ, data];

solve[] := Module[{vals},
  vals = ParallelMap[If[specialQ[#], Total[#], 0] &, data];
  Total[vals]
];
